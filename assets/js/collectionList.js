...
async function updateLotAmort() {
  ...
  const querySnapshot = await getDocs(
    query(collection(db, "Members"), where("memberName", "==", name))
  );
...
async function addCollection() {
  ...
  const querySnapshot = await getDocs(
    query(
      ...
      where("memberName", "==", document.getElementById("memName").value)
    )
  );
  querySnapshot.forEach((doc) => {
  ...
  const checkedRows = Array.from(
    document.querySelectorAll('#colCat table input[type="checkbox"]:checked')
  );
  const dataToAdd = {
  ...
  if (checkedRows.length === 0) {
    ...
    checkedRows.forEach((row) => {
      if (row.parentElement.tagName === "TD") {
        ...
        dataToAdd.Categories.push({
          collectionID: idCell,
          collectionName: nameCell,
          collectionFee: feeCell,
        });
      }
    });
    ...
    if (userConfirmed) {
      try {
        const transactionRef = doc(
          db,
          "CollectionList",
          dataToAdd.TransactionNum
        );
        const snapshot = await getDoc(transactionRef);
        if (snapshot.exists()) {
          alert(
            "Transaction number already exists. Please enter a different number."
          );
...
async function sendSMS() {
  ...
  const sanitizedNumbers = allContactNumbers.map((number) =>
    number.replace(/\D/g, "")
